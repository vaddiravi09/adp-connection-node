...
function ADPAPIConnection(opts) {
	...
	*/
	this.connect = function connect(_opts, cb) {
		...
		if(_opts.keepAlive && typeof auth.on === 'function') {
			debug('Session KeepAlive enabled.');
	...
	*/
	this.refreshToken = function refreshToken(err, token) {
		if(err) {
			debug('Error refreshing access token.');
		} else {
			debug('Refreshed access token.' + token.access_token);
		}
		if(token){
			this.accessToken = token.access_token;
		}
	...
	*/
	...
	@returns {void}
	*/
	this.reconnect = function reconnect(reconnectionObject) {
		if(reconnectionObject.tokenExpiration < new Date()) {
			throw new ConfigurationException({description: 'Token has expired. Reconnection not possible.'});
		}
